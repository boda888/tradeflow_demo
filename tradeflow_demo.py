...
# --- Основной график ---
st.markdown(
    "<h3 style='font-family:Inter, sans-serif; color:#00BCD4;'>📉 BTC Price & Model Predictions</h3>",
    unsafe_allow_html=True
)
st.markdown(
    "<p style='color:#CCCCCC; font-family:Inter, sans-serif;'>Select the date range to explore model signals and accuracy.</p>",
    unsafe_allow_html=True
)

# --- Красивый ползунок выбора даты ---
...
if not no_trade.empty:
    fig.add_trace(go.Scatter(
        ...
        marker=dict(color="#FFD54F", size=6, symbol="circle-open")
    ))
...
fig.update_layout(
    ...
    xaxis=dict(
        rangeslider=dict(visible=False),  # 💥 убрали «чёрную какашку»
        showgrid=False,
        tickfont=dict(color="#B0BEC5", family="Inter, sans-serif", size=11),
    ),
    yaxis=dict(
        showgrid=False,
        tickfont=dict(color="#B0BEC5", family="Inter, sans-serif", size=11),
    ...
    legend=dict(
        ...
        font=dict(family="Inter, sans-serif", color="#CFD8DC", size=11)
    ),
    plot_bgcolor="rgba(17,17,17,0.7)",
    paper_bgcolor="rgba(17,17,17,0.0)",
)
