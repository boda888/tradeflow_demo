...
st.markdown("Smooth playback of model predictions over time (like TradingView replay).")

# Добавим TradingView прямо в этот блок
with st.expander("📊 Open Live TradingView Chart", expanded=False):
    st.markdown("""
    <iframe src="https://s.tradingview.com/widgetembed/?frameElementId=tradingview_btc"
...
if st.button("▶️ Start Simulation"):
    ...
    for i in range(30, len(df), 10):  # обновление каждые 10 точек
        subset = df.iloc[:i]
        sim_fig = go.Figure()
        sim_fig.add_trace(go.Scatter(
            x=subset['datetime'], y=subset['price'],
            mode='lines', name='BTC Price', line=dict(color='gray', width=1)
        ))
        ...
        sim_fig.add_trace(go.Scatter(
            ...
            mode='markers', name='✅ Correct', marker=dict(color='green', size=5)
        ))
        sim_fig.add_trace(go.Scatter(
            x=wrong_live['datetime'], y=wrong_live['price'],
            mode='markers', name='❌ Wrong', marker=dict(color='red', size=5)
        ))
        sim_fig.add_trace(go.Scatter(
            x=no_trade_live['datetime'], y=no_trade_live['price'],
            mode='markers', name='⚪ No Trade', marker=dict(color='orange', size=4)
        ))
